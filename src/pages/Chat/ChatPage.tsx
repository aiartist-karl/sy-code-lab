...
export default function ChatPage() {
  ...
  const [conversations, setConversations] = useState<Conversation[]>([
    { id: 'conv-1', agentId: agent.id, title: '与' + agent.name + '的对话', messages: mockMessages, createdAt: Date.now() - 300000 }
  ]);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0F172A]">
      {/* Header */}
      <div className="flex items-center gap-1.5 px-3 py-2.5 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1E293B] flex-shrink-0 safe-top">
        ...
        <button onClick={() => setShowProject(true)} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 active:bg-slate-200 dark:active:bg-slate-600 relative" title="项目管理">
      ...
      <div className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1E293B] px-3 py-2 safe-bottom flex-shrink-0">
        <div className="flex items-center gap-2 max-w-3xl mx-auto">
          {/* Attachment Button */}
          <button onClick={() => setShowProject(true)} className="p-2.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 active:bg-slate-200 dark:active:bg-slate-600 text-slate-500 transition-colors flex-shrink-0" title="项目文件">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
          </button>
          <input
            ref={inputRef}
            type="text"
